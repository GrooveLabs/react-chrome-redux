...
var Store = function () {
  ...
  function Store(_ref) {
    ...
    var onMessage = function onMessage(message) {
      switch (message.type) {
        case _constants.STATE_TYPE:
          ...
          if (onDisconnect) {
            _this.port.onDisconnect.addListener(onDisconnect);
          }
    ...
    var intervalId = setInterval(function () {
      ...
      _this.port = chrome.runtime.connect(_this.extensionId, { name: portName });

      _this.port.onMessage.addListener(onMessage);
    }, 500);
    ...
    value: function dispatch(data) {
      ...
      return new Promise(function (resolve, reject) {
        try {
          ...
          if (ex.message.match(/Invocation of form runtime\.connect/) && ex.message.match(/doesn't match definition runtime\.connect/)) {
